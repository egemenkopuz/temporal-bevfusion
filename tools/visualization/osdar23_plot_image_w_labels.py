...
def get_args() -> Namespace:
    ...
    parser.add_argument("--only_lidar", type=bool, required=False, default=False)
...
def osdar23_plot_image_w_labels(
    ...
    camera_bbox, lidar_bbox = process_labels(
        img, labels, camera_location, index, only_camera_label, only_lidar_label
    )
    ...
    for id, label in frame_obj["objects"].items():
        if not only_camera_label and "cuboid" in label["object_data"]:
            for x in label["object_data"]["cuboid"]:
                ...
                if len(points_2d) == 8:
                    draw_line(img, points_2d[0], points_2d[1], color, 2)
                    draw_line(img, points_2d[1], points_2d[2], color, 2)
                    draw_line(img, points_2d[2], points_2d[3], color, 2)
                    draw_line(img, points_2d[3], points_2d[0], color, 2)
                    draw_line(img, points_2d[4], points_2d[5], color, 2)
                    draw_line(img, points_2d[5], points_2d[6], color, 2)
                    draw_line(img, points_2d[6], points_2d[7], color, 2)
                    draw_line(img, points_2d[7], points_2d[4], color, 2)
                    draw_line(img, points_2d[0], points_2d[4], color, 2)
                    draw_line(img, points_2d[1], points_2d[5], color, 2)
                    draw_line(img, points_2d[2], points_2d[6], color, 2)
                    draw_line(img, points_2d[3], points_2d[7], color, 2)

        if not only_lidar_label and "bbox" in label["object_data"]:
...
if __name__ == "__main__":
    ...
    osdar23_plot_image_w_labels(
        ...
        only_lidar_label=args.only_lidar,
    )
